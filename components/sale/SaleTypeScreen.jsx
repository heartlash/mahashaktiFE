...
const SaleTypeScreen = () => {
    ...
    const { eggTypeName, eggTypeId } = useGlobalSearchParams()
    ...
    const handleDownload = async () => {
        ...
        for (var sale of saleData) {
            ...
            if(sale.paymentRemarks == null ) sale.paymentRemarks = ''
            data.push([formatDateToDDMMYYYY(sale.saleDate), sale.soldCount, (sale.soldCount / 210).toFixed(2), sale.rate, sale.amount, sale.paidAmount, sale.paymentRemarks, sale.vendorName])
        }

        await getDocument("Sales Report",
            monthNames[selectedMonth - 1] + " " + selectedYear + " " + removeUnderscores(eggTypeName) + " " + vendorName,
            ["Date", "Eggs Sold", "Cartons Sold", "Carton Rate", "Amount", "Paid Amount", "Payment Remarks", "Vendor"],
            data.reverse(),
    ...
    const fetchSaleDataDateRange = async () => {
        ...
        const result = await getSaleDataDateRange(startDate, endDate, eggTypeId);
                ...
                listHeaderComponent={<View>
                    <View className="flex-row items-center mx-2 my-1">
                        ...
                        <View className="flex-1 items-center">
                            <Text className="text-lg font-bold mr-2">{removeUnderscores(eggTypeName)} Sale</Text>
                        </View>
                    </View>
