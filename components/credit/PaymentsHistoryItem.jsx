...
const PaymentsHistoryItem = ({ item, vendorData, isExpanded, onRefreshOnChange, onPress, editItem, setEditItem, setSuccessModalVisible, setFailureModalVisible, setSubmitModalVisible }) => {
    ...
    const handleSavePress = async () => {
        ...
        const result = await updatePayments(updatedItem);
                ...
                <View className="flex-row justify-between mb-3">
                    <View className={`flex-1 pr-4 ${editItem === item ? '' : 'flex-row items-center'}`}>
                        ...
                        {editItem === item ? (
                            ...
                            <Text className="text-gray-700">{item.remarks}</Text>
                        )}
                    </View>
                </View>
